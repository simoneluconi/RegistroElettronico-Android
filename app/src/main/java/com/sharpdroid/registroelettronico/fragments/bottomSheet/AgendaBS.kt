    ...
    private inner class ItemAdapter(val clickListener: Listener) : RecyclerView.Adapter<ItemAdapter.ViewHolder>() {
        ...
        override fun onBindViewHolder(holder: ViewHolder, position: Int) {
            if (event is SuperAgenda) {
                ...
                when (position) {
                    0 -> holder.bindData(if ((event as LocalAgenda).completed_date != 0L) "Non completato" else "Completato",
                            ContextCompat.getColor(context, R.color.greenmaterial_800),
                            if ((event as LocalAgenda).completed_date != 0L) R.drawable.agenda_uncomplete else R.drawable.agenda_complete, ContextCompat.getColor(context, R.color.greenmaterial_800))
